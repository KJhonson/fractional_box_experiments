...
from solver import solve_petsc, sinc_solve, sinc_solve_unified, sinc_solve_standard_unified
...
def solve_neumann_problem_for_beta(mesh, V, beta):
    ...
    if beta == 1.0:
        # Standard Neumann: -Δu + u = f, ∂u/∂n = 0
        u_h, _ = solve_petsc(B, b, V)
    else:
        # Fractional Neumann: (-Δ + I)^β u = f, ∂u/∂n = 0
        u_h, _ = sinc_solve_standard_unified(B, M, b, V, bc=None, beta=beta)
...
def solve_dirichlet_problem_for_beta(mesh, V, beta):
    ...
    if beta == 1.0:
        # Standard Dirichlet: -Δu = f, u = 0 on boundary
        ...
        # Fractional Dirichlet: (-Δ)^β u = f, u = 0 on boundary
        B, _, M, _, facet_tags = build_operator_B(V, bc_type="dirichlet", kappa=1)
        f_expr = lambda x: ufl.sin(ufl.pi * x[0])
        b = assemble_rhs(V, f_expr=f_expr, bc=None)
        bc = fem.dirichletbc(0.0, fem.locate_dofs_topological(V, 1, facet_tags.find(1)), V)
        u_h, _ = sinc_solve_standard_unified(B, M, b, V, bc=bc, beta=beta)
        return u_h
...
def compute_errors_refined(u_h, u_exact_func, Vc, meshc, refine_factor=16, degree_fine=None, bc=None):
    ...
    N_fine = max(refine_factor * N_coarse_dofs, 400)  # ensure "very fine"
...
def test_frac_neumann_comparison(N=100, beta=0.5, m=2, interval=(2, 11), 
    ...
    if 'Sinc' in methods_to_compare:
        ...
        if k_option == "dynamic":
            ...
            u_sinc, _ = sinc_solve_standard_unified(B, M, b, V, bc=None, beta=beta, k=k_use)
        elif isinstance(k_option, (int, float)):
            u_sinc, _ = sinc_solve_standard_unified(B, M, b, V, bc=None, beta=beta, k=k_option)
        else:
            u_sinc, _ = sinc_solve_standard_unified(B, M, b, V, bc=None, beta=beta)
            ...
            for Nconv in convergence_N_list:
                ...
                if method == 'Sinc':
                    if k_option == "dynamic":
                        ...
                        u_c, _ = sinc_solve_standard_unified(B_c, M_c, b_c, V_c, bc=None, beta=beta, k=k_c)
                    elif isinstance(k_option, (int, float)):
                        u_c, _ = sinc_solve_standard_unified(B_c, M_c, b_c, V_c, bc=None, beta=beta, k=k_option)
                    else:
                        u_c, _ = sinc_solve_standard_unified(B_c, M_c, b_c, V_c, bc=None, beta=beta)
                elif method == 'BRASIL':
...
def test_frac_dirichlet_comparison(N=100, beta=0.5, m=2, interval=(1, 11),
    ...
    if 'Sinc' in methods_to_compare:
        ...
        if k_option == "dynamic":
            k_use = dynamic_k(beta, N)
            u_sinc, _ = sinc_solve_standard_unified(B, M, b, V, bc=bc, beta=beta, k=k_use)
        elif isinstance(k_option, (int, float)):
            u_sinc, _ = sinc_solve_standard_unified(B, M, b, V, bc=bc, beta=beta, k=k_option)
        else:
            u_sinc, _ = sinc_solve_standard_unified(B, M, b, V, bc=bc, beta=beta)
        solutions['Sinc'] = u_sinc
        errors['Sinc'] = compute_errors_refined(u_sinc, u_exact, V, mesh, refine_factor=16)
        ...
        for method in methods_to_compare:
            ...
            for Nconv in convergence_N_list:
                ...
                if method == 'Sinc':
                    if k_option == "dynamic":
                        k_c = dynamic_k(beta, Nconv)
                        u_c, _ = sinc_solve_standard_unified(B_c, M_c, b_c, V_c, bc=bc_c, beta=beta, k=k_c)
                    elif isinstance(k_option, (int, float)):
                        u_c, _ = sinc_solve_standard_unified(B_c, M_c, b_c, V_c, bc=bc_c, beta=beta, k=k_option)
                    else:
                        u_c, _ = sinc_solve_standard_unified(B_c, M_c, b_c, V_c, bc=bc_c, beta=beta)
                elif method == 'BRASIL':
...
def compare_standard_methods(N=20, methods_to_compare=['Standard FEM', 'Sinc'], 
    ...
    if 'Sinc' in methods_to_compare:
        ...
        u_sinc, _ = sinc_solve_standard_unified(B, M, b, V, bc=None, beta=1.0)
        solutions['Sinc'] = u_sinc
        errors['Sinc'] = compute_errors_refined(u_sinc, u_exact, V, mesh, refine_factor=16)
        print(f"    Sinc: L²={errors['Sinc']['l2_error']:.2e}, H¹={errors['Sinc']['h1_error']:.2e}, L∞={errors['Sinc']['linf_error']:.2e}")
...
# %%    
    ...
    test_frac_dirichlet_comparison(
        ...
        show_plot=True,
        save_plot=False,
        convergence_N_list=(10, 20, 40, 80, 160, 240),
        scale_factor=3,
        m=3,
        interval=(0.9,51),
        k_option="dynamic",
